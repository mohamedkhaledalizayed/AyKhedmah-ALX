...
import com.land.services.data.model.response.search.Data
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (getItemViewType(position)) {
            ...
            ITEM_TYPE_GENERAL -> if (mList[position] is Data) {
                ...
                mHolder.binding.serviceName.text = data.userData.name
                mHolder.binding.rateBar.rating = data.userData.rate.toString().toFloat()
                if (data.userData.zone.isNullOrEmpty()){
                    mHolder.binding.address.text = data.userData.area.title
                }else{
                    mHolder.binding.address.text = data.userData.area.title + ", " + data.userData.zone?:""
                }

                Picasso.get().load(Constants.STORAGE + data.userData.logo).placeholder(R.drawable.ic_logo__1_).into(mHolder.binding.image)


                holder.itemView.setOnClickListener {
                    onItemClickListener?.setOnItemClickListener(data.userId)
                }
            }
